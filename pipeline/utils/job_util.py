...
def post_remote_pipeline_job_and_wait(pipeline):
    ...
    with contextlib.closing(persist_backend):
        ...
        with contextlib.closing(job_backend):
            ...
            fd = logbook.FlowDetail("flow-of-%s" % my_name,
                                    flow_uuid)
            lb.add(fd)
            with contextlib.closing(persist_backend.get_connection()) as conn:
            ...
            jb = job_backend.post("job-from-%s" % my_name, book=lb)
            print("Posted: %s" % jb)
