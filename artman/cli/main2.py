...
def parse_args(*args):
    ...
    parser_publish.add_argument('--target',
        type=str,
        default=None,
        required=True,
    ...
    parser_publish.add_argument('--dryrun',
        dest='dryrun',
        action='store_true',
...
def normalize_flags(flags, user_config):
    ...
    try:
        ...
        logger.error('Artifact config loading failed with `%s`' % e)
    ...
    if flags.subcommand == 'generate':
        ...
        publishing_config = _get_publishing_config(
            artifact_config, flags.target)
        if publishing_config.type == config_pb2.Artifact.PublishTarget.GITHUB:
            ...
            raise NameError('Publishing type `%s` is not supported yet.' %
                            config_pb2.Artifact.PublishTarget.Type.Name(
                                publishing_config.type))
...
def _run_artman_in_docker(flags):
  ...
  try:
    ...
    logger.error('Artman execution failed. For additional logging, re-run the '
